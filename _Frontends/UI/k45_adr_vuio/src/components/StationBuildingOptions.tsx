...
export const StationBuildingOptionsComponent = ({ entityOrigin, onChanged, response }: Props) => {
    ...
    return <>
        <VR.InfoRow subRow={true} className={rowTheme} left={<><VR.RadioToggle onChange={() => setReference(response.targetEntityToName, response.districtRef)} focusKey={focusKey} className={VR.themeToggleLine.toggle} checked={isDistrictSelected} />{translate("StationBuildingOptions.UseDistrictOpt")}</>} />
        {
