import { Entity, SelectInfoPanelService, SelectedInfoOptions, nameToString } from "@klyte45/adr-commons";
...
export class StationBuildingOptionsComponent extends Component<Props, State>{
    ...
    render() {
        ...
        return <>
            <VR.InfoRow subRow={rowTheme} className={rowTheme} left={<><VR.RadioToggle onChange={() => this.setReference(props.entity, props.response.districtRef)} focusKey={focusKey} className={VR.themeToggleLine.toggle} checked={isDistrictSelected}/>Use district</>} right={<></>} />
            {
                props.response.roadAggegateOptions.map(x =>
                    <VR.InfoRow className={rowTheme} left={<><VR.RadioToggle onChange={() => this.setReference(props.entity, x.entity)} focusKey={focusKey} className={VR.themeToggleLine.toggle} checked={currentVal.Index == x.entity.Index} />Road: {nameToString(x.name)}</>} />)
            }
            {
                props.response.buildingsOptions.map(x =>
                    <VR.InfoRow className={rowTheme} left={<><VR.RadioToggle onChange={() => this.setReference(props.entity, x.entity)} focusKey={focusKey} className={VR.themeToggleLine.toggle} checked={currentVal.Index == x.entity.Index} />Building: {nameToString(x.name)}</>} />)
            }
        </>
    }
}
