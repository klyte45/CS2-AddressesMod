...
namespace BelzontWE
{
    public class GameUIResourceHandlerOverrides : Redirector, IRedirectable
    {
        public void DoPatches(World world)
        {
            ...
            GameManager.instance.onGameLoadingComplete += (_, _) => OnDestroy();
        }

        private static TerrainSystem m_terrainSystem;
        private static WaterSystem m_waterSystem;
        private static SimulationSystem m_simulationSystem;

        private static bool BeforeOnResourceRequest(ref IResourceRequest request, ref IResourceResponse response)
        {
            var url = request.GetURL();
            if (!url.StartsWith("coui://adr.k45/")) return true;
            if (BasicIMod.TraceMode) LogUtils.DoTraceLog("RES URL2 = " + url);
            Texture2D texture = null;
            texture = url switch
            {
                "coui://adr.k45/img/cityTerrain.png" => RenderTextureTo2D(url, m_terrainSystem.heightmap as RenderTexture, x => x.r, MapType.Topographic),
                "coui://adr.k45/img/cityWater.png" => RenderTextureTo2D(url, m_waterSystem.WaterTexture, x => x.r, MapType.Transparency),
                "coui://adr.k45/img/cityWaterPollution.png" => RenderTextureTo2D(url, m_waterSystem.WaterTexture, x => x.a, MapType.Transparency),
                _ => null
            };
            if (texture)
            {
        ...
        private static Dictionary<string, (long, Texture2D)> cachedTextures = new();

        private static Texture2D RenderTextureTo2D(string url, RenderTexture tex, Func<Color, float> mappingFn, MapType mapType)
        {
            if (cachedTextures.TryGetValue(url, out var value) && value.Item1 < m_simulationSystem.frameIndex) return value.Item2;
            if (!tex) return null;
            ...
            if (value.Item2) GameObject.Destroy(value.Item2);
            cachedTextures[url] = (m_simulationSystem.frameIndex + 200, texture);
            return texture;
        }

        public void OnDestroy()
        {
            foreach (var x in cachedTextures.Values)
            {
                GameObject.Destroy(x.Item2);
            }
            cachedTextures.Clear();
        }

        private static Color MapTopographyColor(float rampValue)
        {
